...
import { selectTicket } from "../../redux/features/ticketSlice";
import "./style.css";

const UserTicket = () => {
  const userTicket =useSelector(selectTicket)
    ...
    <div className="user-ticket-container">
      ...
      <div className="user-ticket-information-area">
        <div className="user-ticket-left-area">
          <p>Ad:  <b>{ticket.item.name} </b> </p>
          <p>Soyad:  <b>{ticket.item.surname}</b>  </p>
          <p>Yaş:  <b>{ticket.item.age}</b> </p>
          <p>Tc:  <b>{ticket.item.tc} </b></p>
          <p>Problem:  <b>{ticket.item.problem}</b></p>
        </div>
        <div className="user-ticket-right-area">
          <p>Adres:  <b>{ticket.item.address}</b></p>
          <p>Photo: </p>
          <p>Başvuru Kodu:  <b>{ticket.item.code}</b></p>
          <p>Durum:  <b>{ticket.item.status}</b></p>
        </div>
      </div>
    </div>
  );
